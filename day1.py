...
def GetNumbers(string): 
    soma = 0
    x = re.sub("oneight", "18", string)
    x1 = re.sub("twone", "21", x)
    x2 = re.sub("eightwo", "82", x1)
    x3 = re.sub("one", "1", x2)
    x4 = re.sub("two", "2", x3)
    x5 = re.sub("three", "3", x4)
    x6 = re.sub("four", "4", x5)
    x7 = re.sub("five", "5", x6)
    x8 = re.sub("six", "6", x7)
    x9 = re.sub("seven", "7", x8)
    x10 = re.sub("eight", "8", x9)
    x11 = re.sub("nine", "9", x10)
    numbers = re.findall("\d", x11)
    
    if len(numbers) == 1: 
        x = [numbers[0], numbers[0]]
        numeros_int = [int(numero) for numero in x]
        resultado = int(''.join(map(str, numeros_int)))
        soma += resultado
    elif len(numbers) == 2: 
