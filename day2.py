...
gamesList = ['Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green',
...
array = []
highest_quantities_per_game = []

for game_string in gamesList:
    ...
    if match:
        ...
        highest_quantities_game = {'red': 0, 'green': 0, 'blue': 0}

        for game_component in games:
            matches = re.findall(r"(\d+) (blue|red|green)", game_component)
        
            for quantity, color in matches:                
                # Update the highest quantity for each color in the current game
                if int(quantity) > highest_quantities_game[color]:
                    highest_quantities_game[color] = int(quantity)

        highest_quantities_per_game.append(highest_quantities_game)
        numbers_only = [value for value in highest_quantities_game.values() if isinstance(value, (int, float))]
        result = reduce(lambda x, y: x * y, numbers_only)
        print("Numbers Only:", numbers_only, result)
        array.append(result)


# Print the highest quantities for each color in each game
print("Highest Quantities Per Game:", highest_quantities_per_game)
print(sum(array))